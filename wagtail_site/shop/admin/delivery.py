...
from wagtail_site.shop.conf import app_settings, get_shop_settings
from wagtail_site.shop.admin.order import OrderItemInline
from wagtail_site.shop.models.base.order import OrderItemModel
from wagtail_site.shop.models.base.delivery import DeliveryModel
from wagtail_site.shop.modifiers.pool import cart_modifiers_pool
from wagtail_site.shop.serializers.base.delivery import DeliverySerializer
from wagtail_site.shop.serializers.base.order import OrderDetailSerializer
...
class DeliveryInline(admin.TabularInline):
    ...
    def shop_settings(self, request):
        return get_shop_settings(request)

    def get_readonly_fields(self, request, obj=None):
        readonly_fields = list(super().get_readonly_fields(request, obj))
        if not self.shop_settings(request).override_shipping_method or obj.status == 'ready_for_delivery':
...
class DeliveryOrderAdminMixin:
    ...
    def get_urls(self):
        my_urls = [
            path(r'^(?P<delivery_pk>\d+)/print_delivery_note/$',
